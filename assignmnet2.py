...
def chatgpt_api(query):
    ...
    return "ChatGPT response to: " + query

def watsonx_api(query):
    time.sleep(0.8)
    return "Watsonx response to: " + query

def perplexity_api(query):
    time.sleep(0.3)
    return "Perplexity response to: " + query
...
# Preloaded queries
prebuilt_queries = st.selectbox("Or choose sample query:", [
...
if st.button("Compare Systems"):
    ...
    radar_fig = px.line_polar(st.session_state.metrics, 
                            r=st.session_state.metrics.iloc[0][1:].values,
